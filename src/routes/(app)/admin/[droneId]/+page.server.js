    ...
    try {
        ...
        const envelopeStatuses = await Promise.all(
            drone.Agreement.map(async (agreement) => {
                ...
                if (status.status === 'completed') {
                    try {
                        envelopeDocumentDownloadUrl = await docuSignService.getEnvelopeDocumentDownloadUrl(agreement.docusignEnvelopeId,adminUser.email);
                    } catch (error) {
                ...
                return {
                    ...
                    agreementData: {
                        ...
                        canSign : status.recipients?.signers?.some(
                            ...
                            (signer.status === 'delivered' || signer.status === 'sent')
                        ),
                        declined: {
...
export const actions = {
    default: async ({ request, locals }) => {
        ...
        // Verify admin status - assuming you have an isAdmin field
        ...
        try {
            ...
            const returnUrl = `${request.headers.get('origin')}/admin/${agreement.drone.id}`;

            let recipientOrder = 2;
            ...
            console.error('Admin signing error:', error);
            return fail(500, {
                error: 'Failed to process admin signing request'
            });
        }
